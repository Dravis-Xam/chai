...
function HeaderLogo({ isDarkMode, navigate }) {
  return (
    <div className="flex items-center gap-2 shrink-0 cursor-pointer" onClick={() => navigate('/') }>
      <span className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">Chai</span>
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 pb-4 px-4 pointer-events-none md:hidden">
      <nav
        ...
        className={`relative max-w-md mx-auto rounded-[150px] backdrop-blur-xl transition-all duration-300 pointer-events-auto overflow-visible ${
          isDarkMode ? 'bg-black/40 hover:bg-black/60 border border-white/10' : 'bg-white/40 hover:bg-white/60 border border-black/5'
        }`}
      >
...
<HeaderLogo isDarkMode={isDarkMode} navigate={navigate} />
            <DesktopNav
